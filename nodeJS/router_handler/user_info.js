 ...
 * @LastEditors: sueRimn
 * @LastEditTime: 2022-12-17 14:39:18
 */
...
exports.obtainUserInfo = (req, res) => {
  // SQL查询语句
  const sqlStr3 = `select id, username, nickname, email, user_pic from node_users where id=?`;
...
exports.updateUserInfo = (req, res) => {
  ...
  // SQL查询语句
  const sqlStr4 = `update node_users set ? where id=?`;
  // 查询数据库
...
exports.updatePassword = (req, res) => {
  // 根据id查询用户 判断用户是否存在
  const sqlStr5 = `select * from node_users where id=?`;
  db.query(sqlStr5, req.user.id, (err, results) => {
    ...
    // 定义更新密码SQL语句
    const sqlStr6 = `update node_users set password=? where id=?`;
    // 加密新密码
...
exports.updateAvatar = (req, res) => {
  ...
  const sqlStr7 = `select * from node_users where id=?`;
  // 操作数据库
  db.query(sqlStr7, req.user.id, (err, results) => {
    ...
    // 定义更新头像的 SQL语句
    const sqlStr8 = `update node_users set user_pic=? where id=?`;
